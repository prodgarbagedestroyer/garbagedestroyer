...
const YOUTUBE_CHANNEL_URL = `https://www.youtube.com/@${YOUTUBE_HANDLE.replace("@", "")}`;
...
async function getChannelId(): Promise<string | null> {
  try {
    const res = await fetch(YOUTUBE_CHANNEL_URL, {
      ...
      headers: { "User-Agent": "garbagedestroyer.com/1.0" },
    });
...
async function fetchViaDataAPI(): Promise<YouTubeVideo[] | null> {
  ...
  try {
    const channelRes = await fetch(
      `https://www.googleapis.com/youtube/v3/channels?part=contentDetails&forHandle=${YOUTUBE_HANDLE.replace("@", "")}&key=${apiKey}`,
